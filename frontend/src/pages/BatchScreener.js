  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Phase 2: Partial Results Display */}
        {showPartialResults && partialResults.length > 0 && isProcessing && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-blue-700 text-sm">
                  {partialResults.length} matches found so far • Updates every 10 stocks processed
                  {lastPartialUpdate && (
                    <span className="ml-2">
                      • Last update: {new Date(lastPartialUpdate).toLocaleTimeString()}
                    </span>
                  )}
                </p>
              </div>
            ...
            <div className="max-h-64 overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {partialResults.slice(0, 12).map((stock, index) => (
                      ...
                      <div className="text-right">
                        ...
                        {stock.ppo_hook_display && (
                          <span className={`inline-flex px-1 py-0.5 text-xs rounded ${
                            stock.ppo_hook_type === 'positive' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                          }`}>
                            {stock.ppo_hook_display}
                          </span>
                        )}
              ...
              {partialResults.length > 12 && (
                <div className="text-center mt-3 text-blue-600 text-sm">
                  ... and {partialResults.length - 12} more results
                </div>
              )}
            </div>
          </div>
